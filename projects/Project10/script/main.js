async function fetchTimezoneData(countryName) {
  if (!countryName) {
...
function displayTime(data, htmlElementName) {
  let htmlContent = "";
  let seenKeys = {};
...
const selectedCountries = ["Israel", "China", "USA"];

async function initializeClocks() {
  for (const countryName of selectedCountries) {
    const data = await fetchTimezoneData(countryName);

    setInterval(() => {
      displayTime(data, countryName);
    }, 1000);
  }
}
